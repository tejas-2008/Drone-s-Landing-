...
IBVSController::IBVSController()
    : camera_initialized_(false),
      desired_area_(0),
      desired_centroid_x_(0),
      desired_centroid_y_(0),
      current_centroid_x_(0),
      current_centroid_y_(0),
      current_normalized_area_(0),
...
void IBVSController::setActiveCamera(const CameraIntrinsics& intrinsics) {
    ...
    ROS_INFO("IBVS: Camera intrinsics set - fx: %.2f, fy: %.2f, u0: %.2f, v0: %.2f",
...
void IBVSController::setDesiredFeatures(const VisualFeatures& desired_features, 
    ...
    centered_moments_desired_ = computeCenteredMoments(desired_features_normalized_);

    // Extract centroid
    extractCentroid(desired_features_normalized_, desired_centroid_x_, desired_centroid_y_);

    // Compute desired area
    desired_area_ = arma::accu(centered_moments_desired_(arma::span(2, 2), 
                                                          arma::span(0, 2)));
...
IBVSController::ControlOutput IBVSController::computeControlLaw(
    const VisualFeatures& current_features, 
    ...
    float current_area = arma::accu(centered_moments_current_(arma::span(2, 2), 
                                                               arma::span(0, 2)));
    current_normalized_area_ = depth_estimate * std::sqrt(desired_area_ / 
                                                          std::max(current_area, EPSILON));
    ...
    ROS_DEBUG("IBVS: Error norm: %.4f, Lambda: %.3f, Velocity: [%.3f, %.3f, %.3f, %.3f]",
              output.error_norm, lambda, output.v_x, output.v_y, output.v_z, output.omega_yaw);
...
void IBVSController::computeJacobian(float current_area, float desired_area,
                                     ...
                                     float current_centroid_y,
                                     float depth) {
    jacobian_ = -1.0 * arma::diagmat(arma::ones<arma::vec>(4));

    // Normalized area
    float area_normalized = depth * std::sqrt(desired_area / std::max(current_area, EPSILON));
...
float IBVSController::computeAdaptiveGain(float current_error_norm) {
    // Adaptive gain: lambda = lambda_max - 
    //                (lambda_max - lambda_min) * (error_norm / error_norm_max)
    float lambda_min = 0.5f;
    float lambda_max = 1.25f;
    
    float error_ratio = std::min(current_error_norm / std::max(error_norm_max_, EPSILON), 1.0f);
    float lambda = lambda_max - (lambda_max - lambda_min) * error_ratio;
